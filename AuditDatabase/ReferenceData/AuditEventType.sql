...
BEGIN
    ...
    EventDescription nvarchar(MAX), EventOperationType nvarchar(MAX), [Action] VARCHAR(50));
	...
	USING (VALUES
        ...
        (24, 'EditProviderDetails','A providers details have been edited', 'U'),
        (25, 'EditProvidersAssignments','A providers assignments have been edited', 'U'),
		...
		AS Source(EventId, EventTitle, EventDescription,EventOperationType)
		ON Target.EventId = Source.EventId
		WHEN MATCHED 
				AND EXISTS 
					(		SELECT Target.EventTitle
								  ,Target.EventDescription
								  ,Target.EventOperationType
						EXCEPT 
							SELECT Source.EventTitle
								  ,Source.EventDescription
								  ,Source.EventOperationType
					)
			  THEN UPDATE SET Target.EventTitle = Source.EventTitle,
							  Target.EventDescription = Source.EventDescription
							  ,Target.EventOperationType = Source.EventOperationType
		WHEN NOT MATCHED BY TARGET THEN INSERT(EventId, EventTitle, EventDescription,EventOperationType) 
									   VALUES (Source.EventId, Source.EventTitle, Source.EventDescription, Source.EventOperationType)
